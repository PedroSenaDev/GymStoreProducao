...
serve(async (req) => {
  ...
  try {
    ...
    if (!pixChargeId || !eventType) {
        console.error("Webhook: Dados essenciais ausentes no corpo.", body);
        return new Response("Missing data", { status: 400 });
    }

    console.log(`Webhook recebido: ${eventType} para Pix ID: ${pixChargeId}. Status: ${status}`);

    if (eventType === 'pix_charge_updated' && status === 'PAID') {
        ...
        if (order && order.status === 'pending') {
            ...
            // 4. Decrementar Estoque (Se o pedido foi criado como 'pending', o estoque ainda não foi decrementado)
